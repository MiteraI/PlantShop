    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if (loginedUser != null) {
                ...
                if (cart.containsKey(id)) {  //Similar id found
                    int pastQuantity = cart.get(id);
                    int updatedQuantity = quantity + pastQuantity;
                    cart.put(id, updatedQuantity);
                } else {
                    cart.put(id, quantity);  //No similar id found so add plant with default of 1 plant
                }
                session.setAttribute("cart", cart);
                request.getRequestDispatcher("MainController?action="+Constants.VIEW+"&id="+id).forward(request, response);
            } else {
